...
function showFormat() {
	...
	for (var i = 0; i < commentData['results'].length; i++) {
		...
		span1.innerText = $.base64.decode(commentData['results'][i]['name']) + ':  ';
		span1.setAttribute('class', 'title');
		span.appendChild(span1);

		// 内容
		var span2 = document.createElement('span');
		span2.innerText = $.base64.decode(commentData['results'][i]['content']);
		...
		if (commentData['results'][i]['deletable']=="2") {
